...
class Inbox extends layout {
  ...
  async componentWillReceiveProps(nextProps) {
    if (
      this.props.inbox.listInboxFromManager.items !== nextProps.inbox.listInboxFromManager.items &&
      nextProps.inbox.listInboxFromManager.success &&
    ...
    if (
      this.props.inbox.listInboxIsActive.items !== nextProps.inbox.listInboxIsActive.items &&
      nextProps.inbox.listInboxIsActive.success &&
      this.state.isRefreshActive
    ) {
      await this.setState({ dataIsActive: nextProps.inbox.listInboxIsActive.items });
      await this.setState({ isRefreshActive: false, isLoadDataActive: false });
    }
    ...
    if (
      ...
      await this.setState({ loadingMoreInboxActive: false, isRefreshActive: false, isLoadDataActive: false });
    }

    //data to manager
    // if (
    //   this.props.inbox.listInboxToManager.items !== nextProps.inbox.listInboxToManager.items &&
    //   nextProps.inbox.listInboxToManager.success &&
    //   this.state.isRefreshToManager
    // ) {
  ...
  async _onRefresh() {
    ...
    await this.setState({ isRefresh: true, pageInbox: 1, isLoadData: true });
    await this._getListInbox(1);
  }
  ...
  async _onRefreshIsActive() {
    ...
    await this.setState({ isRefreshActive: true, pageInboxActive: 1, isLoadDataActive: true });
    await this._getListInboxIsActive(1);
  }
