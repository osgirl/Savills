...
class TabComplete extends PureComponent {
        ...
        {this.state.isLoadData === false ? (
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <ItemWorkOrder {...this.props} item={item} index={index} action={() => this.gotoDetail(item, 0)} />
            )}
            ...
            ListEmptyComponent={() => {
              return <EmptyItemList message={languages.WO_EMPTY_LIST_COMPLETE} />;
            }}
          />
        ) : (
  ...
  _getList() {
    ...
    this.props.actions.workOrder.getWorkOrderListActive(accessTokenApi, id, this.state.pageCount, languages);
  }
}
