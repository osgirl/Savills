...
import { TouchableOpacity, View, Animated, ScrollView, StyleSheet, Text } from 'react-native';
...
export type Props = {
  ...
  tabBarStyle: Object,
  renderTab: (tab: TabType, page: number, isTabActive: boolean, onPressHandler: Function, onTabLayout: Function) => any
};
...
class TabBar extends Component<Props, State> {
  ...
  handleScrolling = (event: { value: number }) => {
    ...
    if (this.scrollOffsetsCollection) {
      const scrollOffset = this.scrollOffsetsCollection && this.scrollOffsetsCollection._interpolation(value);
      if (this.scrollView) {
  ...
  checkMeasures = () => {
    if (this.state.renderUnderline && this.tabContainerLayout.width !== 0 && this.scrollContainerLayout.width !== 0) {
  ...
  renderTab = (tab: TabType, page: number, isTabActive: boolean, onPressHandler: Function, onTabLayout: Function) => {
    ...
    return (
      <TouchableOpacity
        style={
          ([
            styles.tab,
            this.props.tabMargin && {
              marginLeft: this.props.tabMargin
            }
          ],
          {
            flexDirection: 'row',
            ...
            paddingTop: 20,
            borderBottomWidth: 1,
            borderBottomColor: isTabActive ? '#FFF' : '#d2d2d2',
  ...
  render() {
    const { style, backgroundColor, tabBarStyle, tabMargin, scrollContainerStyle, tabs } = this.props;
    return (
      <View
        style={([styles.tabs, { backgroundColor }, style, tabBarStyle], { marginLeft: 15, marginRight: 15 })}
        onLayout={this.measureTabsContainer}
      >
        <ScrollView
          horizontal
          contentContainerStyle={[styles.scrollContainer, tabMargin && { paddingRight: tabMargin }, scrollContainerStyle]}
