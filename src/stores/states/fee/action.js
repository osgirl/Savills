...
export function getListUserFees(accessTokenAPI, language = 'en', fullUnitCode = '') {
    return {
        ...
        payload: {
            api: Configs.API + `/api/Fees/fees/userfees?fullUnitCode=${fullUnitCode}&statusIds=1&statusIds=3&page=1&culture=${language}`,
            method: 'GET',
...
export function getListHistory(accessTokenAPI, language = 'en', fullUnitCode = '', page = 1, pageSize = 10) {
    return {
        ...
        payload: {
            api: Configs.API + `/api/Fees/fees/receiptfilters?fullUnitCode=${fullUnitCode}&page=${page}&pageSize=${pageSize}&culture=${language}`,
            method: 'GET',
            token: accessTokenAPI
        }
    }
